...
export default class SeedCidadesPA implements Seeder {
  public async run(factory: Factory, connection: Connection): Promise<void> {
    //* -> get estado_id
    const estadoRepo = getRepository(Estado);

    const estado = await estadoRepo
      .findOne({ where: { codigo_ibge: '15' } })
      .catch(err => {
    ...
    //* -> xlsx to json
    const file = XLSX.readFile(`${__dirname}/xlsx/seed_cidades_pa.xlsx`);
    const sheet = file.SheetNames;

    //* -> looping estado_id
    for (let i = 2; i <= 145; i += 1) {
